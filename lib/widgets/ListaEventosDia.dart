...
class _ListaEventosDiaState extends State<ListaEventosDia> {
  ...
  Future<void> _cargarEventosDelDia() async {
    ...
    try {
      ...
      final eventosDocs = querySnapshot.docs
          .map((doc) => Eventos.fromFirestore(doc.data()))
      ...
      for (var evento in eventosDocs) {
        final color = evento.usuarioRef == null ? Colors.black : coloresUsuarios[evento.usuarioRef!.id] ?? Colors.black;
      ...
      resultado.sort((a, b) => (a['evento'] as Eventos).timestamp.compareTo((b['evento'] as Eventos).timestamp));
  ...
  Widget build(BuildContext context) {
    ...
    if (eventosConColor.isEmpty) return const Center(child: Text("No hay eventos para mostrar"));
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final hora = evento.timestamp.toDate();
        ...
        return Padding(
          ...
          child: ListTile(
            ...
            subtitle: evento.descripcion != null
                ? Text(evento.descripcion!, style: TextStyle(color: Colors.grey.shade700))
                : null,
            trailing: Text(horaFormateada, style: TextStyle(fontSize: 18, color: color)),
          ),
        );
      },
    );
